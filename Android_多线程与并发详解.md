...
3. [线程同步机制](#3-线程同步机制)
   ...
   - 3.2 [volatile](#32-volatile)
      - 3.2.1 [volatile 防止指令重排序](#321-volatile-防止指令重排序dcl单例模式)
...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  synchronized 的三大保证：
  ─────────────────────────────────────────────────────────────────────────
  1. **原子性**：同步块内的代码作为一个整体被执行，不会被线程调度打断
     - 例如：`i++` 在 synchronized 块中执行时，不会被其他线程的 `i++` 打断

  2. **可见性**：当线程退出 synchronized 块时，会释放锁并把工作内存的修改刷新到主内存；其他线程进入 synchronized 块时会强制从主内存读取最新值
     - 线程 A 释放锁 → 强制刷新共享变量到主内存
     - 线程 B 获取锁 → 强制从主内存读取共享变量

  3. **有序性**：在 synchronized 块内部的代码，编译器不能进行指令重排序
     - synchronized 代码块中的代码按照编写顺序执行
        ...
        if (instance == null) {  // 第一次检查
            synchronized (Singleton.class) {
                if (instance == null) {  // 第二次检查
                    instance = new Singleton();  // 这里有指令重排序问题！
                }
            }
        }
        return instance;
...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  可能被重排序为（编译器/CPU 优化）：
  1. 分配内存
  2. instance 指向这块内存（此时不为 null）
  3. 调用构造函数初始化对象

  ─────────────────────────────────────────────────────────────────────────
  
  问题场景：
  线程 A 进入 synchronized 块，执行了 1→2→3，但还没完成构造函数时，
  线程 B 看到 instance != null，直接使用了一个未初始化完成的对象！
```
...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  volatile 通过内存屏障（Memory Barrier）实现：
  ...
  ─────────────────────────────────────────────────────────────────────────
  结论：volatile 确保了"读取 instance"之前，构造函数一定已经执行完毕
```

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                   为什么不用 synchronized 代替 volatile？                      │
└─────────────────────────────────────────────────────────────────────────────┘

  问题：如果去掉 volatile，只用 synchronized

  public static Singleton getInstance() {
      if (instance == null) {  // 线程 B 可能看到未初始化完成的对象
          synchronized (Singleton.class) {
              if (instance == null) {
      ...
      return instance;  // 线程 B 可能返回半初始化的对象！
  }
  ...
  synchronized 只能保证代码块内的有序性，不能保证代码块外的操作

  ─────────────────────────────────────────────────────────────────────────
  解决方案：
  • volatile：轻量级，添加内存屏障防止重排序
  • 或去掉第一次检查：每次都加锁（性能差）
```
