...
| 18 | [Android_多线程与并发详解.md](./Android_多线程与并发详解.md) | 310KB | ⭐ Java线程/同步/CAS/并发集合/线程池/Kotlin协程/Flow（12章）|
| 19 | [Android_SystemUI_详解.md](./Android_SystemUI_详解.md) | 281KB | ⭐ StatusBar/NavBar/AOD/通知/Keyguard 源码 (Android 16 AOSP) |
...
| 22 | [Android_PMS_包管理.md](./framework/Android_PMS_包管理.md) | 11KB | ⭐ APK安装/组件解析/权限管理/签名验证（9章）|
| 23 | [Android_InputManager_输入系统.md](./framework/Android_InputManager_输入系统.md) | 9KB | ⭐ EventHub/InputChannel/触摸按键事件分发（10章）|
| 24 | [Android_进程与线程调度.md](./framework/Android_进程与线程调度.md) | 26KB | ⭐ CFS/优先级/Binder线程池/Looper/线程池（9章）|
| 25 | [Android_系统启动流程.md](./framework/Android_系统启动流程.md) | 26KB | ⭐ Init/Zygote/SystemServer/BootComplete（8章）|
...
| 核心机制 | 10 篇 | 1.40 MB | Binder/Handler/虚拟机/SystemUI/AndroidX/多线程并发 ⭐ |
...
1. ✅ **Android_多线程与并发详解.md** (310KB, 12章, 3000+行)
   - Java 线程基础（Thread创建/状态生命周期/优先级/守护线程）
   - 线程同步机制（synchronized/volatile/wait-notify/Lock/ReadWriteLock/Condition/StampedLock）
   - Java 并发工具类（CountDownLatch/CyclicBarrier/Semaphore/Exchanger/Phaser）
   - 原子类与 CAS（AtomicInteger/AtomicReference/LongAdder/CAS原理/ABA问题）
   - 并发集合（ConcurrentHashMap/CopyOnWriteArrayList/BlockingQueue/ThreadLocalMap）
   - 线程池（ThreadPoolExecutor/执行流程/饱和策略/参数配置）
   - Kotlin 协程原理（suspend状态机/Dispatchers/Context/Scope/Flow/Channel）
   - Android 线程模型（主线程职责/Binder线程池/Process优先级）
   - 并发设计模式（Thread-Per-Message/Worker Thread/Producer-Consumer/Pipeline/Actor）
   - 性能与反模式（伪共享/过度同步/常见反模式）
   - 面试高频问题（10+道）
...
*Generated by OpenClaw | 2026-03-24*
